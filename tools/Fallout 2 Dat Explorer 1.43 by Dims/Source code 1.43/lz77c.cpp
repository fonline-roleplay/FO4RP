...
{
   ULONG i, j, Len;
   ULONG buff_size = 0x0000FFFF;// Defaul buffer size = 64K.
   HANDLE h_out;
   String TempPackedFile = WinTempDir + "\\lz66pck.$$$";

   h_out = CreateFile(TempPackedFile.c_str(),
		GENERIC_WRITE,
		FILE_SHARE_WRITE,
		NULL,
		CREATE_ALWAYS,
   ...
   while(Len != 0)
   {
      if (Len > buff_size)
      {
         ReadFile(h_in, buf, buff_size, &j, NULL);
         WriteFile(h_out, buf, j, &i, NULL);
      }
      else
      {
         ReadFile(h_in, buf, Len, &j, NULL);
         WriteFile(h_out, buf, j, &i, NULL);
      }
      Len -= j;
   }
